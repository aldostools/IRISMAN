...
int mm_insert_htab_entry_355deh(uint64_t va_addr, uint64_t lpar_addr, uint64_t prot, uint64_t * index)
{
    ...
    int result = lv1_insert_htab_entry_355deh(0, hpte_group, hpte_v, hpte_r, HPTE_V_BOLTED, 0,
                                       &hpte_index, &hpte_evicted_v, &hpte_evicted_r);

    if ((result == 0) && (index != 0)) *index = hpte_index;

    return (int) result;
}

int mm_map_lpar_memory_region_355deh(uint64_t lpar_start_addr, uint64_t ea_start_addr, uint64_t size, uint64_t page_shift,
                              uint64_t prot)
{
    uint32_t i, result;
    ...
    {
        result = mm_insert_htab_entry_355deh(MM_EA2VA(ea_start_addr), lpar_start_addr, prot, 0);
